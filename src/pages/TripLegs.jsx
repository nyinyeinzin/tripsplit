...
const orderStops = (a, b) => a.day.localeCompare(b.day) || (a.scheduled_time || "99:99").localeCompare(b.scheduled_time || "99:99") || a.order_index - b.order_index || a.created_at.localeCompare(b.created_at);
...
export default function TripLegs({ trip, user, onTripUpdated }) {
  ...
  const load = useCallback(async () => {
    const [stopResult, legResult, participantResult, memberResult] = await Promise.all([
      supabase.from("stops").select("id,name,day,scheduled_time,order_index,created_at").eq("trip_id", trip.id),
      supabase.from("legs").select("id,from_stop_id,to_stop_id,estimated_minutes,estimated_distance_km,estimated_cost,computed_at").eq("trip_id", trip.id),
  ...
  function openManual(from, to, leg) {
    ...
    setManual({ minutes: String(leg?.estimated_minutes ?? ""), distance: String(leg?.estimated_distance_km ?? ""), cost: String(leg?.estimated_cost ?? "") });
    setError("");
  }
  ...
  return <section className="trip-legs"><div className="trip-stops-heading"><div><p className="trips-overline">Getting around</p><h2>Transport & shared fares</h2></div></div>
    ...
    {!pairs.length ? <p className="trips-status">Add at least two stops to see transport between them.</p> : pairs.map(([from, to]) => {
      ...
      return <article className="trip-leg-card" key={key}><div className="trip-leg-route"><span><Route size={18} /></span><div><strong>{from.name} → {to.name}</strong><small>{to.day}</small></div></div>
        <div className="trip-leg-metrics"><span><Clock3 size={16} /> {leg?.estimated_minutes != null ? `${leg.estimated_minutes} min` : "Time unknown"}</span><span><MapPin size={16} /> {leg?.estimated_distance_km != null ? `${leg.estimated_distance_km} km` : "Distance unknown"}</span></div>
        ...
        {editing === key && <form className="trip-leg-manual" onSubmit={(event) => saveManual(event, from, to, count)}><label>Minutes <input required type="number" min="0" step="1" value={manual.minutes} onChange={(event) => setManual({ ...manual, minutes: event.target.value })} /></label><label>Distance (km) <input required type="number" min="0" step="0.01" value={manual.distance} onChange={(event) => setManual({ ...manual, distance: event.target.value })} /></label><label>Fare per vehicle ({trip.currency}) <input required type="number" min="0" step="0.01" value={manual.cost} onChange={(event) => setManual({ ...manual, cost: event.target.value })} /></label><button type="submit" disabled={busy === "manual"}>Save estimate</button><button type="button" onClick={() => setEditing(null)}>Cancel</button></form>}
      </article>;
    })}
  </section>;
}
