...
import { calculateBalances, calculateSettlements, equalShares, toCents } from "../utils/ledger";
...
export default function TripExpenses({ trip, user }) {
  ...
  const total = expenses.reduce((sum, expense) => sum + toCents(expense.amount), 0);
  ...
  async function saveExpense(event) {
    ...
    let shares;
    if (splitType === "custom") {
      shares = selectedUsers.map((id) => ({ user_id: id, amount_owed: toCents(custom[id] || 0) / 100 }));
      if (shares.some((share) => !Number.isSafeInteger(toCents(share.amount_owed)) || share.amount_owed < 0) || shares.reduce((sum, share) => sum + toCents(share.amount_owed), 0) !== amountCents) return setError("Custom shares must add up exactly to the expense amount.");
    } else {
      shares = equalShares(amountCents, selectedUsers);
    }
